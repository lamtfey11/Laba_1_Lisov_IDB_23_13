...
Выбор формата хранения (JSON или XML) реализован через паттерн Strategy. Оба модуля, `json_job.py` и `xml_job.py`, реализуют один и тот же набор функций с одинаковыми именами и назначением, но разной реализацией внутри. В начале работы, в зависимости от выбора пользователя, в переменную `handler` кладётся нужный модуль:
...
Дальше весь остальной код `main.py` вызывает `handler.add_human(...)`, `handler.save_json(...)` и так далее, не зная и не проверяя, какой формат выбран. Ветвление по формату происходит один раз, в момент выбора `handler`, а не размазано по всему коду через `if формат == "json": ... else: ...` в каждом месте работы с файлом.
